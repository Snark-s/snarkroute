...
from app.video_registry import VideoModelRegistry  # noqa: E402


def main() -> int:
    parser = argparse.ArgumentParser(description="Explicitly download a pinned local video upscale model")
    parser.add_argument("--model", required=True)
    parser.add_argument("--model-dir", type=Path, default=Path(os.getenv("LOCAL_UPSCALE_MODEL_DIR", "./models")))
    parser.add_argument("--dry-run", action="store_true")
    args = parser.parse_args()
    model = VideoModelRegistry.load(ModelRegistry.load()).get(args.model)
    if not model.resource or model.framewise_model_id:
        print("framewise models reuse existing local_upscale weights; use download_models.py", file=sys.stderr)
        return 2
    ...
    print(f"model={model.id} bytes={model.resource.size_bytes} available_free_bytes={gate.available_bytes}")
    ...
    try:
        download_verified_file(
            ...
            user_agent="SnarkRoute-local-video-upscale/0.1",
        )
