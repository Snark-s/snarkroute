...
export class SnarkRouteGatewayClient {
  ...
  async models(): Promise<{ models: VideoModel[]; modelCount: number; familyCount: number; diagnosticsUrl: string }> { const body = await this.request<{ models?: VideoModel[]; modelCount?: number; familyCount?: number; diagnosticsUrl?: string }>("/api/models/for-node/polza.video.generate?image=1&video=0&audio=0"); const models = body.models ?? []; return { models, modelCount: body.modelCount ?? models.length, familyCount: body.familyCount ?? 0, diagnosticsUrl: body.diagnosticsUrl ?? "/api/models/for-node/polza.video.generate/debug?image=1&video=0&audio=0" }; }
