...
import { capabilityForOperation, countModelFamilies, filterModelsForOperation, maximumImageInputs, modelSupportsPrompt, operationLabels, operationsForModels, outputMediaTypeForOperation } from "../api/catalog";
...
export function App() {
  ...
  const [bridgeToken, setBridgeToken] = useState(() => localStorage.getItem("snarkroute.after-effects.bridge-token") ?? "");
  const mcpStatus = useAeMcpBridge(serverUrl, bridgeToken);
  ...
  const [operation, setOperation] = useState<GenerationOperation>("image-to-video");
  const models = useMemo(() => filterModelsForOperation(catalogModels, operation), [catalogModels, operation]);
  const [modelId, setModelId] = useState(""), [prompt, setPrompt] = useState(""), [parameters, setParameters] = useState<Record<string, unknown>>({});
  const [imageSource, setImageSource] = useState<"current-composition-frame" | "external-file">("current-composition-frame"), [externalImagePath, setExternalImagePath] = useState("");
  ...
  const requiresImage = operation.startsWith("image-to-") || ["image-editing", "inpainting", "outpainting", "image-upscale"].includes(operation);
  ...
  useEffect(() => { if (selectedModel) setParameters(Object.fromEntries(selectedModel.parameters.map((field) => [field.id, field.default ?? (field.type === "boolean" ? false : "")]))); }, [selectedModel?.id]);
  ...
  async function generate() {
    ...
    if (modelSupportsPrompt(selectedModel) && selectedModel.parameters.some((field) => field.id.toLowerCase() === "prompt" && field.required) && !prompt.trim()) return dispatch({ type: "phase", phase: "failed", message: "Prompt is required." });
    if (requiresImage && imageSource === "external-file" && !externalImagePath) return dispatch({ type: "phase", phase: "failed", message: "Choose an external image file." });
  ...
  return <main>
    <section><h2>AE MCP</h2><div className={`status ${mcpStatus === "connected" ? "ok" : "error"}`}>AE MCP {mcpStatus}</div><label className="field"><span>Bridge token</span><input type="password" value={bridgeToken} onChange={(event) => setBridgeToken(event.target.value)} onBlur={() => localStorage.setItem("snarkroute.after-effects.bridge-token", bridgeToken)} /></label></section>
    <section><h2>Connection</h2><label className="field"><span>Server URL</span><input value={serverUrl} onChange={(event) => setServerUrl(event.target.value)} /></label><div className={connected ? "status ok" : "status error"}>SnarkRoute server: {connecting ? "connecting…" : connected ? "connected" : "disconnected"}</div>{connection?.error && <div className="error-text">{connection.error}</div>}<button onClick={() => void connect()} disabled={connecting}>Reconnect</button></section>
    ...
    <section><h2>Model</h2><select value={modelId} onChange={(event) => setModelId(event.target.value)}>{models.map((model) => <option key={`${model.nodeType}:${model.id}`} value={model.id}>{model.displayName} · {model.provider}</option>)}</select>{selectedModel && <small>{selectedModel.availability.status} · output {outputMediaTypeForOperation(operation)} · up to {maximumImageInputs(selectedModel)} image input(s)</small>}<div className="diagnostics">Executable models: {models.length} · Families: {countModelFamilies(models)}</div></section>
    <section><h2>Inputs</h2>{requiresImage ? <><label className="field"><span>Source image · required</span><select value={imageSource} onChange={(event) => setImageSource(event.target.value as typeof imageSource)}><option value="current-composition-frame">Current composition frame</option><option value="external-file">External image file</option></select></label>{imageSource === "external-file" && <><button onClick={async () => setExternalImagePath(await host.selectExternalImage() ?? externalImagePath)}>Choose image…</button><small>{externalImagePath || "No file selected"}</small></>}</> : <small>No media inputs are required.</small>}</section>
    {selectedModel && modelSupportsPrompt(selectedModel) && <section><h2>Prompt</h2><textarea rows={4} value={prompt} onChange={(event) => setPrompt(event.target.value)} /></section>}
    ...
    <section><button className="primary" onClick={() => void generate()} disabled={!connected || !selectedModel || isBusy(job.phase)}>Generate</button></section>
    <section><h2>Current job</h2><div className={`status ${job.phase === "failed" ? "error" : ""}`}>Stage: {job.phase}{job.message ? ` · ${job.message}` : ""}</div>{(exportDiagnostic ?? details?.inputFrameExport) && <ExportDiagnostics diagnostic={(exportDiagnostic ?? details?.inputFrameExport)!} />}</section>
