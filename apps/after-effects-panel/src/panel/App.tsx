...
export function App() {
  ...
  const mcpBridge = useAeMcpBridge(serverUrl, bridgeToken);
  ...
  return <main>
    <section><h2>AE MCP</h2><div className={`status ${mcpBridge.status === "connected" ? "ok" : "error"}`}>AE MCP {mcpBridge.status}</div><label className="field"><span>Bridge token</span><input type="password" value={bridgeToken} onChange={(event) => { const value = event.target.value; localStorage.setItem("snarkroute.after-effects.bridge-token", value); setBridgeToken(value); }} /></label><div className="diagnostics"><div>WebSocket URL: {mcpBridge.webSocketUrl || "invalid"}</div><div>Last bridge error: {mcpBridge.lastError || "none"}</div><div>Last close: {mcpBridge.lastCloseCode ?? "none"} · {mcpBridge.lastCloseReason || "no reason"}</div><div>Last attempt: {mcpBridge.lastAttemptAt || "none"}</div><div>Reconnect attempts: {mcpBridge.reconnectAttempts}</div></div></section>
