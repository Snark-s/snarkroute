...
import { capabilityForOperation, countModelFamilies, filterModelsForOperation, frontendExclusionReason, maximumImageInputs, modelSupportsPrompt, operationLabels, operationsForModels, outputMediaTypeForOperation } from "../api/catalog";
...
export function App() {
  ...
  const generateBlockReason = !connected ? "Connect to the SnarkRoute server." : !selectedModel ? "Choose an executable model." : isBusy(job.phase) ? "The current job is still running." : currentInputErrors[0] ?? currentParameterErrors[0] ?? "";
