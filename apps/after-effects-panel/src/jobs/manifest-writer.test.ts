...
describe("CEP Node manifest writer", () => {
  ...
  it("uses APPDATA for an unsaved project", async () => {
    ...
    expect(await generationDirectory(unsaved(), runtime)).toBe(path.join(root, "Пользовательские данные", "SnarkRoute Generations", "Videos"));
    expect(await generationDirectory(unsaved(), "image", runtime)).toBe(path.join(root, "Пользовательские данные", "SnarkRoute Generations", "Images"));
  });
  ...
  it("falls back to TEMP when APPDATA mkdir fails", async () => {
    ...
    expect(await generationDirectory(unsaved(), instrumented.runtime)).toBe(path.join(root, "Temp", "SnarkRoute Generations", "Videos"));
  });
