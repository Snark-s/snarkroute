import type { GenerationMetadata, ManifestWriteDiagnostic, MediaKind, ProjectFileContext } from "../types";
...
export async function generationDirectory(context: ProjectFileContext, mediaKindOrRuntime: MediaKind | ManifestNodeRuntime = "video", suppliedRuntime?: ManifestNodeRuntime): Promise<string> {
  const mediaKind = typeof mediaKindOrRuntime === "string" ? mediaKindOrRuntime : "video";
  const runtime = typeof mediaKindOrRuntime === "string" ? (suppliedRuntime ?? loadCepNodeRuntime()) : mediaKindOrRuntime;
  const child = mediaKind === "image" ? "Images" : "Videos";
  if (context.saved && context.projectDirectory) return ensureDirectory(runtime.path.join(context.projectDirectory, "SnarkRoute Generations", child), runtime);
  const appData = runtime.env.APPDATA || context.appDataPath;
  const temp = runtime.env.TEMP || runtime.env.TMP || context.tempPath;
  if (appData) {
    try { return await ensureDirectory(runtime.path.join(appData, "SnarkRoute Generations", child), runtime); }
  ...
  return ensureDirectory(runtime.path.join(temp, "SnarkRoute Generations", child), runtime);
}
