...
describe("downloaded result import", () => {
  it("imports FileSource footage, replaces the located layer source, and writes metadata", async () => {
    ...
    expect(host.importResultFootage).toHaveBeenCalledWith(baseJob.outputPath, "Kling 3.0 · 2026-07-17 19-42-10", "video");
  ...
  it("imports the video even when placeholder creation originally failed", async () => {
    ...
    expect(result).toMatchObject({ status: "completed", importedFootage: { importedItemId: 120 } });
  ...
  it("retries only import/replacement without starting another generation", async () => {
    ...
    expect(failed).toMatchObject({ status: "completed_with_warning", failedStage: "replacing_placeholder", outputPath: baseJob.outputPath, warning: "AE refused replaceSource" });
  ...
  it("imports every image output and uses the first as the placeholder replacement", async () => {
    const imageJob: PersistedJob = { ...baseJob, outputMediaType: "image", outputPath: "C:\\one.png", outputs: [{ kind: "image", role: "primary", index: 0, path: "C:\\one.png", filename: "one.png", mimeType: "image/png" }, { kind: "image", role: "alternate", index: 1, path: "C:\\two.png", filename: "two.png", mimeType: "image/png" }] };
    const importResultFootage = vi.fn(async (path: string) => ({ ...imported, resultPath: path, importedItemId: path.includes("one") ? 120 : 121 }));
    const replacePlaceholderSource = vi.fn(async () => ({ ok: true, sourceReplaced: true, placeholderDeleted: false, placeholderCompositionId: 17, newSourceId: 120, newSourceType: "FileSource" }));
    const result = await importDownloadedResult(imageJob, { ...metadata, outputMediaType: "image" }, { host: { importResultFootage, replacePlaceholderSource, writeGenerationMetadata: vi.fn() } });
    expect(importResultFootage).toHaveBeenCalledTimes(2); expect(replacePlaceholderSource).toHaveBeenCalledWith(baseJob.placeholder, 120, "Kling 3.0 · Generated"); expect(result.outputs).toMatchObject([{ importedItemId: 120, usedAsPlaceholderReplacement: true }, { importedItemId: 121, usedAsPlaceholderReplacement: false }]);
  });
});
